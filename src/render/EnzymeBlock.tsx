...
const EnzymeBlock = forwardRef<EnzymeBlockRef, CodeBlockRendererProps>(
	(props, ref) => {
		const {
			app,
			enzymeAgent,
		...
		const getDigestStartLine = (): EditorPosition => {
			...
			let endOfCodeFenceLine =
				context.getSectionInfo(contentRef.current)?.lineEnd ?? 0
		...
		const handleSourcesButtonClick = () => {
			...
			Promise.all(
				sources.map(async (source) => {
					...
					const sourceEl = sourcesContentRef.current?.createEl('div', {
						cls: 'enzyme-source'
					})
					if (!sourceEl) {
						return
		...
		useEffect(() => {
			const handleClickOutside = (event: MouseEvent) => {
				if (
					sourcesContentRef.current &&
					!sourcesContentRef.current.contains(event.target as Node)
				) {
			...
			const sourcesContentEl = document.body.createEl('div', {
		...
		return (
			<>
				<div ref={contentRef} className="enzyme-container">
					<button
						className="enzyme-digest-button"
						onClick={handleDigestButtonClick}
					>
						Digest
					</button>
					{sources.length > 0 && (
						<button
							...
							className="enzyme-sources-button"
...
export const renderCodeBlockRenderer = (
